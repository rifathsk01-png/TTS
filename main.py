...
def _generate_voice(text: str, language: str, gender: str) -> bytes:
    """
    Generate TTS audio using pyttsx3 and return WAV bytes.
    Male/Female voice selection is system-dependent.
    Bangla voice works only if your system has Bangla-supported TTS voice installed.
    """
    engine = pyttsx3.init()
    ...
    for voice in voices:
        ...
        voice_langs = " ".join([str(lang).lower() for lang in getattr(voice, "languages", [])])

        is_bangla_voice = (
            "bn" in voice_langs
            or "bangla" in voice_name
            or "bengali" in voice_name
            or "bengali" in voice_id
            or "bangla" in voice_id
        )
        ...
        is_male_voice = (
            ...
            or "mark" in voice_name
            or "alex" in voice_name
        )

        language_match = is_bangla_voice if language == "bangla" else is_english_voice
        gender_match = is_female_voice if gender == "female" else is_male_voice

        if language_match and gender_match:
...
async def tts_gender_chosen(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"<i>Minimum {MIN_WORDS} word required.</i>",
...
async def tts_receive_text(message: Message, state: FSMContext):
    ...
    audio_file = BufferedInputFile(audio_bytes, filename="voice.wav")
